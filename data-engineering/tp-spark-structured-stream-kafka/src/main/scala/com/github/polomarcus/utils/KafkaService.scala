...
object KafkaService {
  ...
  /** will return, we keep some kafka metadata for our example, otherwise we
    * would only focus on "news" structure \|-- key: binary (nullable = true)
    * \|-- value: binary (nullable = true) \|-- topic: string (nullable = true)
    * : \|-- partition: integer (nullable = true) : \|-- offset: long (nullable
    * \= true) : \|-- timestamp: timestamp (nullable = true) : \|--
    * timestampType: integer (nullable = true) \|-- news: struct (nullable =
    * true) \| |-- title: string (nullable = true) \| |-- description: string
    * (nullable = true)
    */
  def read(
      startingOption: String = "startingOffsets",
      partitionsAndOffsets: String = "earliest"
  ): Dataset[NewsKafka] = {
    logger.warn(s"""
    ...
    val dataframeStreaming = spark.readStream
      .format("kafka")
      ...
      ) // this only applies when a new query is started and that resuming will always pick up from where the query left off
      .load()

    // We need to read our binary value to our class object News
    val newsDataset: Dataset[NewsKafka] = dataframeStreaming
      .withColumn(
        "news", // nested structure with our json
        from_json(
  ...
  def debugStream[T](ds: Dataset[T], completeOutputMode: Boolean = true) = {
    val outputMode = if (completeOutputMode) { OutputMode.Complete() }
  ...
  def writeToParquet(ds: Dataset[NewsKafka]) = {
    ds.writeStream
      .queryName("Write to Parquet")
      .format("parquet")
      .option("path", ConfService.PATH_PARQUET)
      .outputMode("append")
      .option("checkpointLocation", ConfService.PATH_CHECKPOINT)
