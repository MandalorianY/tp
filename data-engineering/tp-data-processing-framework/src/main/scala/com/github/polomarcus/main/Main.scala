...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    // This is our Spark starting pointZ
    ...
    val filteredNews = NewsService.filterNews(newsDatasets)

    // Enrich the filtered dataset with climate metadata
    val enrichedFilteredNews =
      NewsService.enrichNewsWithClimateMetadata(filteredNews)
    ...
    // Tips: use a groupBy
    enrichedFilteredNews.groupBy("containsWordGlobalWarming").count().show()
    ...
    enrichedFilteredNews.createOrReplaceTempView("news")

    spark
      .sql("SELECT * FROM news WHERE containsWordGlobalWarming = true")
      .show()
    ...
    enrichedFilteredNews.write
      .partitionBy("date", "media")
      .mode("overwrite")
      .parquet("./enriched_news.parquet")
